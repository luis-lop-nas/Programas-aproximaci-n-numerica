...
def metodo_newton_raphson(f, df, x0, tolerancia=1e-3, max_iteraciones=100):
    ...
    for iteracion in range(max_iteraciones):
        ...
        # Si la derivada es cero, el metodo no puede continuar (division por cero)
        if abs(dfx) < 1e-15:
            print(f"\nError: La derivada es cero en x = {x_actual}. No se puede continuar.")
            return None
...
def ingresar_funcion():
    ...
    try:
        ...
        print(f"Error al calcular la derivada: {e}")
        return None, None, None
...
def main():
    ...
    metodo_newton_raphson(f, df, x0, tolerancia, max_iteraciones)
